    ...
    <div>
      ...
      >
            ...
            <v-dialog v-model="duplicationDialog" max-width="500px">
              <v-card dark class="glass2">
                ...
                <v-card-actions>
                  ...
                  <v-btn color="green outlined" :loading="dialogLoading" text @click="duplicateItemConfirm">Ja Duplizieren</v-btn>
                  <v-spacer></v-spacer>
                </v-card-actions>
              </v-card>
            </v-dialog>
        ...
        <template v-slot:item.image="{ item }">
                <div class="p-2">
                  <v-img v-if="item.images.length != 0" :src="item.images[0].src" :alt="item.name" height="75px" width="75px"></v-img>
                </div>
        </template>
        ...
        <template v-slot:no-data>
            <div id="err">Es wurde kein Ergebniss für <strong>"{{ suche }}"</strong> gefunden</div>
        </template>
    ...
    methods: {
      ...
      SearchCategory() {
        if(!this.searchCategory == "") {
          this.products = this.$store.state.products.filter((product) => {
            return this.searchCategory.name.toLowerCase().split(' ').every(v => product.categories.filter((category) => {
      ...
      save () {
        ...
        if (this.editedIndex > -1) {
          ...
          axios.patch(`https://bindis-schaulaedle.de/wp-json/wc/v3/products/${this.editedItem.id}/?consumer_key=ck_04911d593cc006c24c8acbe6ebc4b1e55af6ae33&consumer_secret=cs_9b1bd2702eb5fc89f5b55d40fa8dafe622c2bddc`,
          this.editedItem)
          .then(Response => {
            this.$store.state.products[this.editedIndex] = Response.data
            this.$store.state.products.splice(1, 0, this.$store.state.products.splice(this.editedIndex, 1)[0])
